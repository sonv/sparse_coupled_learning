...
if __name__ == "__main__":
    ...
    # target_edges_list = np.logspace(np.log10(50), np.log10(10000), num=20, dtype=int)
    target_edges_list = np.logspace(np.log10(50), np.log10(100), num=1, dtype=int)
    ...
    print("Topology & " + " & ".join(f"{n}" for n in target_edges_list) + " \\\\")
    for topology in topologies:
        log_costs = np.log(results[topology]['costs'])
        mean_log_costs = np.mean(log_costs, axis=1)
        std_log_costs = np.std(log_costs, axis=1)
        mean_costs = np.exp(mean_log_costs)
        mean_edges = np.mean(results[topology]['edges'], axis=1)
        print(f"{topology} & {' & '.join(f'{cost:.4f} ({std:.4f}) [{edge:.0f}]' for cost, std, edge in zip(mean_costs, std_log_costs, mean_edges))} \\\\")
